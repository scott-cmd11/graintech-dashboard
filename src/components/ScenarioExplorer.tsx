...
import { SlidersHorizontal, TrendingUp, RotateCcw, TrendingDown } from "lucide-react";
        ...
        <div className="grid gap-6 lg:grid-cols-[1.3fr_1fr]">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
            ...
            <div className={`rounded-xl border p-4 bg-gradient-to-br transition-all duration-300 ${getValueColor(outputs.riskReductionScore, 100, 'risk')}`}>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-2">
                <AnimatedCounter value={`${outputs.riskReductionScore}/100`} />
              </p>
              {riskImprovement !== 0 && (
                <div className="flex items-center gap-1 text-xs mt-2">
                  {riskImprovement > 0 ? (
                    <>
                      <TrendingUp className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />
                      <span className="text-emerald-600 dark:text-emerald-400 font-medium">+{riskImprovement.toFixed(0)}% improvement</span>
                    </>
                  ) : (
                    <>
                      <TrendingDown className="w-3 h-3 text-amber-600 dark:text-amber-400" />
                      <span className="text-amber-600 dark:text-amber-400 font-medium">{riskImprovement.toFixed(0)}% change</span>
                    </>
                  )}
                </div>
              )}
              <p className="text-xs text-gray-600 dark:text-gray-500 mt-2">
                Higher = fewer disputes
              </p>
            </div>
          </div>
        </div>
